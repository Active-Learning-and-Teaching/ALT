...
import {handleQuiz1} from '../Sheets/PushClassDataToSheet';
import { quizHandler } from '../Sheets/AppendData';
    ...
    if (action === 'stop') {
      ...
      await kbc.getQuestion(this.state.course.passCode).then(values => {
        ...
        kbc
          ...
          .then(r => {
            console.log('update kbc.startQuestion');
          });
      });
    } else {
          ...
          if (values === null) {
            ...
            kbc
              ...
              .then(r => {
                ...
                console.log('updated kbc.getQuestion');
            ...
            this.state.resultPage === false ? (
              ...
              <ScrollView>
                <View style={[{paddingRight: 10, paddingLeft: 10}]}>
                  ...
                  onPress={ () => {
                    Linking.openURL('https://docs.google.com/spreadsheets/d/' + this.state.course.spreadsheetURL + '/edit#gid=0');
                  }}
                />
                  </View>
                </View>
                  ...
                  await Kbc.getTiming(this.state.course.passCode).then(async value => {
                    if (value != null) {
                      await this.setState({
                        correctAnswer: value['correctAnswer'],
                        date: value['startTime'],
                        quizNumber: value['questionCount']
                      });
                    }
                  });
                  console.log(this.state.date,this.state.time,this.state.correctAnswer, this.state.quizNumber,this.state.typeofQuiz,"logging params");
                  if (this.state.quizNumber != 1)
                  await quizHandler(this.state.course,this.state.date,this.state.time,this.state.correctAnswer, this.state.quizNumber,this.state.typeofQuiz);
                  else await handleQuiz1(this.state.course);
                  this.setState({
                    resultPage: true,
                  });
                  this.checkEmailSent().then(r => {
