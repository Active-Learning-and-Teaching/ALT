  ...
  student
    .once('value', function(snapshot) {
      snapshot.forEach(el => {
        ...
        const thisStudent = admin
          .app()
          .database(url)
          .ref('InternalDb/Student/' + studentKey + '/courses');
        thisStudent
          .once('value', function(snapshot) {
            snapshot.forEach(el => {
              if (el.val() === courseKey) {
    ...
    .once('value', function(snapshot) {
      snapshot.forEach(el => {
        ...
        const thisStudent = admin
          .app()
          .database(url)
          .ref('InternalDb/Faculty/' + facultyKey + '/courses');
        thisStudent
          .once('value', function(snapshot) {
            snapshot.forEach(el => {
              if (el.val() === courseKey) {
...
function removeFromFacultyList(key) {
  const faculty = admin
    .app()
    .database(url)
    .ref('InternalDb/Faculty/' + key + '/courses');
  faculty
    .once('value', function(snapshot) {
      snapshot.forEach(el => {
    ...
    try {
      ...
      if (!_data.emailResponse && _data.startTime != '') {
        const Noitfier = {
          notification: {
            title: 'FeedBack',
            body: `A new feedBack form has been posted by instructor ${
              _data.instructor
            } !`,
          },
          topic: _data.passCode, // Passing the path params along with the notification to the device. [optional]
